        ...
        <div class=" responsive">
          <!-- <q-markup-table dense bordered wrap-cells>
             ...
             <tr v-for="v in rows" :key="v.id">
               ...
               <td>
                 <q-btn dense round flat color="yellow" @click="editRow(v)" icon="edit"><q-tooltip>Editar</q-tooltip></q-btn>
          ...
          <q-card-section class="q-pt-xs">
            ...
            >
              ...
              <div v-if="dato.tipocliente=='1'">
                ...
                <q-select outlined v-model="dato.legalidad" :options="['CON LICENCIA','SIN LICENCIA']"
                          label="Legalidad Tributaria"/>
                <q-select outlined v-model="dato.categoria" :options="['GENERAL','SIMPLIFICADO','SIN NIT']"
                          label="Categoria"/>
                <q-input
                  type="text"
                  outlined
      ...
      this.$axios.get(process.env.API + '/cumple').then(res => {
        ...
        res.data.forEach(el => {
          ...
          if (valor == el.tipocliente) {
            ...
            if (el.estado == 'ACTIVO')
              this.days.push({
                cumple: date.formatDate(Date.now(), 'YYYY') + '/' + date.formatDate(fecha, 'MM') + '/' + date.formatDate(fecha, 'DD'),
                titular: el.titular,
                local: el.local
              })
            this.clientes = {};
