<template>
  <q-page class="q-pa-xs">
    <div class="row">
      <div class="col-2">
        <q-input dense outlined v-model="fecha3" label="Fecha Ini" type="date"/>
      </div>
      <div class="col-2">
        <q-input dense outlined v-model="fecha4" label="Fecha fin" type="date"/>
      </div>
      <div class="col-2">
...
<!--        <pre>{{materialSelect}}</pre>-->
      ...
      <div class="col-2 flex flex-center">
        <q-btn color="green" label="EXCEL" @click="exportTable2" no-caps :loading="loading"/>
      </div>
      <div class="col-12">
        <q-table
          dense
          ...
          <template v-slot:body-cell-costo="props">
            <q-td :props="props">
              <template v-if="$store.state.login.almacenCostoSubtotal">
                <q-badge v-if="$store.state.login.preciounitario"
                         :color="props.row.subtotal>props.row.deuda?'red':'green'">{{ props.row.costo }}
                </q-badge>
              </template>
            </q-td>
          </template>
          <template v-slot:body-cell-saldocant="props">
            <q-td :props="props" :style="props.row.saldocant>=1?'font-weight: bold; font-size: 18px;':''"
                  :color="props.row.saldocant>=1?'black':'red'">
              {{ props.row.saldocant }}
            </q-td>
          </template>
          ...
          <template v-slot:body-cell-subtotal="props">
            <q-td :props="props">
              <template v-if="$store.state.login.almacenCostoSubtotal">
                <q-badge v-if="$store.state.login.preciounitario"
                         :color="props.row.subtotal>props.row.deuda?'red':'green'">{{ props.row.subtotal }}
                </q-badge>
              </template>
            </q-td>
          </template>
          ...
          <template v-slot:body-cell-comentario="props">
            <q-td :props="props">
              <q-btn icon="list" color="indigo" dense v-if="props.row.comentario!=null && $store.state.login.user.id==1"
                     @click="verObs(props.row.comentario)"/>
            </q-td>
          </template>
          <template v-slot:body-cell-observacion="props">
            <q-td :props="props">
              <q-btn icon="list" color="purple" dense v-if="props.row.observacion!=null"
                     @click="verObs(props.row.observacion)"/>
            </q-td>
          </template>
          <template v-slot:body-cell-opcion="props">
            <q-td key="opcion" :props="props">
              ...
              <q-btn-dropdown label="Opciones" color="grey-3" text-color="black" no-caps size="10px" dense>
                <q-list>
                  <q-item clickable v-if="$store.state.login.pagoalmacen && props.row.deuda<props.row.subtotal"
                          @click="pagarcompra(props.row)">
                    <q-item-section avatar>
                      <q-icon name="paid" size="20px"/>
                    </q-item-section>
                    <q-item-section>Realizar Pago</q-item-section>
                  </q-item>
                  <q-item clickable v-if="$store.state.login.almacenHistorialPago && props.row.logcompras.length>0"
                          @click="listpago(props.row)">
                    <q-item-section avatar>
                      <q-icon name="point_of_sale" size="20px"/>
                    </q-item-section>
                    <q-item-section>Ver Pagos</q-item-section>
                  </q-item>
                  <q-item clickable v-if="$store.state.login.egresoMaterial && props.row.retiro<props.row.cantidad"
                          @click="retirarRow(props.row)">
                    <q-item-section avatar>
                      <q-icon name="o_download" size="20px"/>
                    </q-item-section>
                    <q-item-section>Realizar retiro</q-item-section>
                  </q-item>
                  <q-item clickable v-if="$store.state.login.egresoMaterial && props.row.recuentos.length>0"
                          @click="listrecuento(props.row)">
                    <q-item-section avatar>
                      <q-icon name="list" size="20px"/>
                    </q-item-section>
                    <q-item-section>Ver Retiros</q-item-section>
                  </q-item>
                  <q-item clickable v-if="$store.state.login.editalmacen " @click="editcompra(props.row)">
                    <q-item-section avatar>
                      <q-icon name="o_edit" size="20px"/>
                    </q-item-section>
                    <q-item-section>Editar</q-item-section>
                  </q-item>
                  <q-item clickable v-if="$store.state.login.editalmacen && props.row.estado=='POR PAGAR'"
                          @click="delcompra(props.row)">
                    <q-item-section avatar>
                  ...
                  <q-separator/>
                  <q-item clickable v-if="$store.state.login.user.id==1" @click="comentarioAdmin(props.row)">
                    <q-item-section avatar>
                      <q-icon name="info" size="20px"/>
                    </q-item-section>
                    <q-item-section>Comentario admin</q-item-section>
                  </q-item>
    ...
    <q-dialog v-model="providerDialog">
      <q-card style="width: 700px; max-width: 90vw;">
        ...
        <q-card-section>
          <q-form v-if="providerOptions=='create' || providerOptions=='edit'"
                  @submit="providerOptions === 'create' ? providerCreate() : providerUpdate()">
            <div class="row">
              <div class="col-5">
                <q-input outlined dense v-model="provider.nombre" label="Nombre" lazy-rules
                         :rules="[val => !!val || 'Campo requerido']"/>
              </div>
              <div class="col-4">
                <q-input outlined dense v-model="provider.razon" label="Razón Social" lazy-rules
                         :rules="[val => !!val || 'Campo requerido']"/>
              </div>
              <div class="col-3">
            ...
            <q-card-section class="row justify-end">
              <q-btn :loading="loading" color="red" no-caps icon="o_delete" label="Cancelar" dense
                     @click="providerOptions = 'list'"/>
              <q-btn :loading="loading" no-caps icon="o_save" dense type="submit"
                     :label="providerOptions === 'create' ? 'Guardar' : 'Actualizar'"
                     :color="providerOptions === 'create' ? 'green' : 'yellow-7'"/>
            </q-card-section>
          </q-form>
          <q-table :loading="loading" :rows-per-page-options="[0]" dense :rows="providers"
                   v-if="providerOptions === 'list'" :columns="providerColumns" row-key="name">
            <template v-slot:top-right>
              <q-btn color="green" icon="add_circle_outline" label="Nuevo" no-caps
                     @click="providerOptions = 'create'; provider = {}"/>
              <q-input outlined dense v-model="filter" debounce="300" placeholder="Buscar...">
                <template v-slot:append>
                  <q-icon name="search" class="cursor-pointer"/>
                </template>
            ...
            <template v-slot:body-cell-actions="props">
              <q-td key="opcion" :props="props">
                <q-btn dense round flat color="accent" icon="edit"
                       @click="providerOptions = 'edit'; provider = props.row"/>
      ...
      <q-card style="width: 500px; max-width: 90vw;">
        ...
        <q-card-section>
          <q-form @submit="materialOptions === 'create' ? materialCreate() : materialUpdate()">
            <div class="row">
              <div class="col-12">
                <q-input outlined dense v-model="material.nombre" label="Nombre" lazy-rules
                         :rules="[val => !!val || 'Campo requerido']"/>
              </div>
              <!--              <div class="col-12">-->
              ...
              <!--              </div>-->
              <div class="col-6">
                <q-select outlined dense v-model="material.unid" label="Unidad" :options="unidades" lazy-rules
                          :rules="[val => !!val || 'Campo requerido']"/>
              </div>
              <div class="col-6">
                <q-input outlined type="number" dense v-model="material.min" label="Minimo" hint=""
                         :rules="[val => !!val || 'Campo requerido']">
                  <q-tooltip>
    ...
    <q-dialog v-model="dialog_add">
      <q-card style="width: 800px; max-width: 80vw;">
        ...
        <q-card-section class="q-pt-xs">
          <div class="row">
            <div class="col-6">
              <q-select dense outlined v-model="material" :options="materials" label="Material" option-label="nombre"
                        option-value="id"/>
            </div>
            <div class="col-6">
              <q-select dense outlined v-model="provider" :options="providers" label="Proveedor" option-label="razon"
                        option-value="id"/>
            </div>
            <div class="col-2">
              <q-input dense outlined type="text" v-model="compra.cantidad" label="Cantidad"/>
            </div>
            <div class="col-2">
              <template v-if="$store.state.login.almacenCostoSubtotal">
                <q-input dense outlined type="number" step="0.01" v-model="compra.costo" label="Costo U"
                         v-if="$store.state.login.preciounitario"/>
              </template>
            </div>
            ...
            <div class="col-2">
              <q-input dense outlined type="text" v-model="compra.lote" label="Lote"/>
            </div>
            <div class="col-3">
              <q-input dense outlined type="date" v-model="compra.fechaven" label="Fecha Vencimiento"/>
            </div>
            <div class="col-3" v-if="$store.state.login.user.id==1">
              <q-input dense outlined type="text" v-model="compra.comentario" label="Comentario"/>
            </div>
            <div class="col-3">
              <q-input dense outlined type="text" v-model="compra.observacion" label="Observacion"/>
            </div>
            <div class="col-12 text-right">
              <q-btn label="Añadir" no-caps color="green" icon="add_circle" @click="agregarcompra"
                     v-if="compraOptions === 'create'"/>
            </div>
            <!--            <div class="col-12">-->
            <!--              <pre>{{compra}}</pre>-->
            <!--            </div>-->
            <div class="col-12" v-if="compraOptions === 'create'">
              <q-markup-table>
                ...
                <tr v-for="(d,index) in compras " :key="index">
                  ...
                  <td class="text-right">
                    <q-btn flat dense color="red" icon="o_delete" @click="eliminar(index)"/>
                  </td>
                </tr>
                </tbody>
              </q-markup-table>
    ...
    <q-dialog v-model="dialoglistretiros" full-width>
      <q-card>
        <q-card-section class="row items-center">
          <q-table :title="'RETIROS TOTAL: ' + totalRetiro" :rows="retiros" :columns="colretiros" row-key="name" flat
                   :rows-per-page-options="[0]">
            <template v-slot:body-cell-op="props">
              <q-td key="op" :props="props">
                <q-btn dense round flat color="red" icon="delete" v-if="$store.state.login.almacenHistorialPago"
                       @click="delrecuento(props.row)"/>
              </q-td>
            </template>
          </q-table>
        </q-card-section>
...
export default {
  ...
  data() {
    return {
      ...
      dialog_add: false,
      //inicio de anio
      // fecha3:moment().startOf('year').format('YYYY-MM-DD'), 18 meses atras
      fecha3: moment().subtract(18, 'months').format('YYYY-MM-DD'),
      // fecha4:moment().endOf('year').format('YYYY-MM-DD'), hoy
  ...
  created() {
    ...
    this.colcompra = [
      ...
      },
      {
        ...
        align: 'center',
        label: 'CANTIDAD',
        field: row => row.cantidad + ' - ' + row.tretiro,
        sortable: true
      },
      ...
      {name: 'lote', align: 'center', label: 'LOTE', field: 'lote', sortable: true},
      {
        ...
        align: 'center',
        label: 'FECHA VEN',
        field: row => moment(row.fechaven).format('DD/MM/YYYY'),
        sortable: true
      },
    ...
    if (user.id == 1) {
      this.colcompra.push({
        ...
        align: 'center',
        label: 'OBSERVACION ADMIN',
        field: 'observacionAdmin',
        sortable: true
      })
    }
    this.providersGet()
  ...
  methods: {
    getMaterialSelect() {
      console.log(this.materialSelect.id)
      if (this.materialSelect.id == 0) {
        this.comptodo = this.comptodoAll
        return false
      }
      this.comptodo = this.comptodoAll.filter((item) => {
        return item.material_id == this.materialSelect.id
      })
    },
    verObs(cadena) {
      this.$q.dialog({
    ...
    genreporte() {
      ...
      this.$api.post(process.env.API + '/repalmacen', {
        id: this.material2.id,
        fecha1: this.fecha1,
        fecha2: this.fecha2
      }).then(res => {
        let mc = this
